...
| `OPENAI_API_KEY` | your OpenAI key (Banner Builder + LP Materials) |

`PLATFORM_SECRET_KEY` is auto-generated by Render; `ADMIN_EMAIL`,
`PLATFORM_ENV=production`, and `PLATFORM_COOKIE_SECURE=true` come from the Blueprint
(each independently refuses the weak dev admin and turns API docs off — set both).
...
- **App login** — every route except the login requires a valid session cookie
  (JWT, ~8h). Set a strong `ADMIN_PASSWORD_HASH` (step 1).
