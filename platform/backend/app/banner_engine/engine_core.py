...
def post_images_edits(api_key, prompt, master_png_path, openai_size, model, quality, timeout,
                      mask_png_path=None, extra_image_paths=None):
    ...
    edit; `extra_image_paths` (optional) appends further image[] entries
    (e.g. a style reference) — the mask applies to the FIRST image only.
    (Both power the banner Edit workspace's text-correction path.)
    """
    ...
    for p in extra_image_paths or []:
        if os.path.exists(p):
            with open(p, "rb") as fh:
                extra_images.append((os.path.basename(p), fh.read()))
    ...
    for fname, data in extra_images:
        _file("image[]", fname, data)
...
def generate_png(*, api_key, prompt, mode, openai_size, model="gpt-image-2",
                 ...
                 extra_image_paths=None, timeout=180, max_retries=2,
                 base_backoff=8, sleep=time.sleep, on_attempt=None):
