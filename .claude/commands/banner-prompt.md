...
# /banner-prompt — Prompt generator only (no rendering, no Figma) v1.0

## What this does

Same creative reasoning as `/banner` — but stops before any tool call. Produces:
...
## Source of truth — reuses /banner

This command reuses **§ Design Framework**, **§ Composition Guide**, **§ Visual Prompt Template**, and the **DO NOT RENDER list** from `.claude/commands/banner.md`. Read those sections — do not re-derive them here.

Everything that applies to `/banner` Phase 0 + Phase 1 applies here unchanged:
- Language auto-detection (incl. dialect cues)
- Cultural safety + market-aware imagery rules
- RTL composition + RTL typography
- Money element priority list
- 3-tier hierarchy, 3-size max, max 2 typefaces
- Hex-coded palette discipline
...
CTA is **optional** here (unlike `/banner` which requires it). Prompt-only mode tolerates a CTA-less hero banner.

---

## Phase 0 — auto-detect LANGUAGE

Identical to `/banner` Phase 0. Same label set (`pt-BR`, `pt-PT`, `es-LATAM`, `es-ES`, `English`, `Arabic`, `Hebrew`, `Urdu`, `Farsi`, `Pashto`, `th-TH`, `tr-TR`, …). If `lang:` override is supplied, honor it.

Surface the detection line (same format as `/banner` v1.6):
...
Apply `/banner`'s Composition Guide in full:

1. **Internally answer the 9-decision checklist** from `.claude/commands/banner.md`:
   1. Realistic customer · 2. Hero subject · 3. Setting · 4. Lighting · 5. Composition direction (LTR/RTL) · 6. Money element · 7. Color palette (with hex codes) · 8. Typography by script · 9. CTA treatment (or "none" if no CTA supplied)
2. **Fill the § Visual Prompt Template** from `.claude/commands/banner.md` with concrete decisions
3. **Verify length ≤ 2,000 chars** after filling. If over, tighten — drop redundant adjectives, merge sentences. Never drop content that names a specific subject, position, color, or line of copy.

If a `style:` hint was provided, bias the 9-decision answers toward that hint while still satisfying the framework (cultural safety, RTL rules, money element priority remain non-negotiable).

If `variant: N` is supplied as a follow-up, look up variant N from the previous turn's alternatives list, substitute its axis choices (imagery style / subject / setting / lighting / palette) into the checklist, then recompose.
...
```
📋 Approach: <one-line approach name, e.g. "Photorealistic, warm-lit Brazilian male subject, navy/orange palette, home-office setting">

📐 Canvas: 1200×1200 (1:1) · Language: <LANGUAGE> · Direction: <LTR|RTL>
...
- **Prompt length ≤ 2,000 chars** per /banner's constraint. Verify after filling.
