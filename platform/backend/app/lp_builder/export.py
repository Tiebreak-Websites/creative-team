...
def compose_page(project: dict, sections_map: Dict[str, dict], mode: str,
                 ...
                 hide_scrollbars: bool = False) -> Dict[str, str]:
    ...
    # Global font: an explicitly chosen Google font wins EVERYWHERE — including
    ...
    if family:
        ...
        fams = [family] + ([noto] if noto and noto != family else [])
        q = "&".join("family=" + f.replace(" ", "+") + ":wght@400;500;600;700;800" for f in fams)
        stack = ",".join(f"'{f}'" for f in fams) + ",system-ui,-apple-system,'Segoe UI',Roboto,sans-serif"
        # The font rides the CSS payload (@import must stay first), NOT a head
        # <link>: the canvas morphs style content in place and never re-syncs
        # head tags, and the ZIP's styles.css gets it for free the same way.
        fonts_link = ""
        css = (f"@import url('https://fonts.googleapis.com/css2?{q}&display=swap');\n"
               + css
               + "\n/* page font override */\n"
               + f":root{{--lp-font:{stack}}}\n"
               + "body,body *{font-family:var(--lp-font) !important}")
    else:
    ...
    # SEO extras (Page settings ▸ SEO). og:image/favicon values ride the same
    # resolver as body images, so canvas serving and ZIP collection both work.
    seo = project.get("seo") or {}
    og_title = _esc(seo.get("og_title") or "") or title
    og_desc = _esc(seo.get("og_description") or "") or desc
    canonical_raw = (seo.get("canonical") or "").strip()
    og_image_raw = resolve_img(seo["og_image"]) if seo.get("og_image") else ""
    if og_image_raw and canonical_raw and not og_image_raw.startswith(("http://", "https://", "data:")):
        # crawlers need an absolute og:image — anchor the zip-relative asset
        # path to the canonical URL when one is configured
        og_image_raw = urljoin(canonical_raw, og_image_raw)
    og_image = _esc(og_image_raw)
    favicon = _esc(resolve_img(seo["favicon"])) if seo.get("favicon") else ""
    seo_lines = ['<meta property="og:type" content="website">',
                 f'<meta property="og:title" content="{og_title}">']
    if og_desc:
        seo_lines.append(f'<meta property="og:description" content="{og_desc}">')
    if og_image:
        seo_lines.append(f'<meta property="og:image" content="{og_image}">')
        seo_lines.append('<meta name="twitter:card" content="summary_large_image">')
    if favicon:
        seo_lines.append(f'<link rel="icon" href="{favicon}">')
    if canonical_raw:
        seo_lines.append(f'<link rel="canonical" href="{_esc(canonical_raw)}">')
    if seo.get("robots_index") is False:
        seo_lines.append('<meta name="robots" content="noindex, nofollow">')
    seo_html = "\n".join(seo_lines)
...
{seo_html}
{'<style>html{scrollbar-width:none}::-webkit-scrollbar{width:0;height:0;display:none}</style>' if hide_scrollbars else ''}
{fonts_link}
{style_or_link}
